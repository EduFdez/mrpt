...
bool SubgraphMatcher::evalUnaryConstraints(Plane &plane1, Plane &plane2, PbMap &trgPbMap, bool useStructure)
{
    ...
    //  if(plane1.bDominantColor)
    {
        ...
        //
        if( BhattacharyyaDist(plane1.hist_H, plane2.hist_H) > configLocaliser.hue_threshold ){
#if _VERBOSE
            cout << "FALSE  Hist_H " << BhattacharyyaDist(plane1.hist_H, plane2.hist_H) << " > " << configLocaliser.hue_threshold << "\n";
#endif
            //    ;
...
bool SubgraphMatcher::evalUnaryConstraintsOdometry(Plane &plane1, Plane &plane2, PbMap &trgPbMap, bool useStructure)
{
    ...
    //  if(plane1.bDominantColor)
    {
        ...
        if( BhattacharyyaDist(plane1.hist_H, plane2.hist_H) > configLocaliser.hue_threshold ){
#if _VERBOSE
            cout << "Hist_H false_ " << BhattacharyyaDist(plane1.hist_H, plane2.hist_H) << " > " << configLocaliser.hue_threshold << "\n";
#endif
            //    ;
            //    tCondition3 = pcl::getTime();
...
bool SubgraphMatcher::evalUnaryConstraints2D(Plane &plane1, Plane &plane2, PbMap &trgPbMap, bool useStructure)
{
    ...
    //  if(plane1.bDominantColor)
    {
        ...
        if( BhattacharyyaDist(plane1.hist_H, plane2.hist_H) > configLocaliser.hue_threshold ){
#if _VERBOSE
            cout << "Hist_H false " << BhattacharyyaDist(plane1.hist_H, plane2.hist_H) << " > " << configLocaliser.hue_threshold << "\n";
#endif
            //    ;
            //    tCondition3 = pcl::getTime();
...
//    ASSERT_( subgraph_planes.size() == arrangement.size() || arrangement.size() > 0 );
//    //ASSERT_( std::max_element(arrangement) < arrangement.size() );
