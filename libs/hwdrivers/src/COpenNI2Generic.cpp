...
std::ostream& printOni2DevInfoStr(std::ostream& out, const openni::DeviceInfo& info, int tab = 0);
...
// #define NI2_LOG_STREAM std::cerr  //Show logs.
#define NI2_LOG_STREAM std::stringstream() //Hide logs.
	...
	m_depth_format(openni::PIXEL_FORMAT_DEPTH_1_MM)
{
	if(numInstances == 0){
		if(openni::OpenNI::initialize() != openni::STATUS_OK){
			...
			NI2_LOG_STREAM << __FUNCTION__ << std::endl << " Initialized OpenNI2." << std::endl;
		}
	}
	numInstances++;
}
...
unsigned int COpenNI2Generic::getNumDevices()const{
...
int COpenNI2Generic::getConnectedDevices()
{
	...
	NI2_LOG_STREAM << __FUNCTION__ << std::endl;
	NI2_LOG_STREAM << " Get device list. " << numDevices << " devices connected." << std::endl;

	// Search new devices.
	std::set<int> newDevices;
	for(unsigned i=0; i < numDevices; i++){
		const openni::DeviceInfo& info = oni2InfoArray[i];
		NI2_LOG_STREAM << "  Device[" << i << "]" << std::endl;
		printOni2DevInfoStr(NI2_LOG_STREAM, info, 3) << std::endl;
		bool isExist = false;
	...
	if(getNumDevices() == 0){
		NI2_LOG_STREAM << " No devices connected -> EXIT" << endl;
	}else{
		NI2_LOG_STREAM << " " << getNumDevices() << " devices were found." << endl;
	}
...
void COpenNI2Generic::open(unsigned sensor_id)
{
	...
	NI2_LOG_STREAM << __FUNCTION__ << std::endl;
	NI2_LOG_STREAM << " open[" << sensor_id << "] ..." << std::endl;

	if(isOpen(sensor_id)){
		NI2_LOG_STREAM << " The sensor [" << sensor_id << "] is already opened" << std::endl;
		return;
	}
	vDevices[sensor_id]->open(m_width, m_height, m_fps);
	NI2_LOG_STREAM << vDevices[sensor_id]->getLog() << std::endl;
	NI2_LOG_STREAM << " Device [" << sensor_id << "] ";
	if(vDevices[sensor_id]->isOpen()){
		NI2_LOG_STREAM  << " open successfully." << std::endl;
	}else{
		NI2_LOG_STREAM  << " open failed." << std::endl;
	}
	...
	unsigned sensor_id )
{
	...
	if(vDevices[sensor_id]->getNextFrameRGB(rgb_img, timestamp, there_is_obs, hardware_error) == false){
		NI2_LOG_STREAM << __FUNCTION__ << ":Error [" << sensor_id << "]th Sensor." << std::endl;
		NI2_LOG_STREAM << " " << vDevices[sensor_id]->getLog() << std::endl;
	}
#else
	THROW_EXCEPTION("MRPT was built without OpenNI2 support")
...
std::ostream& printOni2DevInfoStr(std::ostream& out, const openni::DeviceInfo& info, int tab){
	std::string space;
	for(int i = 0;i < tab;++i){
		space += " ";
	}
	out << space << "name="    << info.getName()        << std::endl;
	out << space << "uri="     << info.getUri()         << std::endl;
	out << space << "vendor="  << info.getVendor()      << std::endl;
	out << space << "product=" << info.getUsbProductId();
	return out;
}
...
//
COpenNI2Generic::CDevice::CDevice(const openni::DeviceInfo& info, openni::PixelFormat rgb, openni::PixelFormat depth):m_mirror(true), m_info(info){
...
bool COpenNI2Generic::CDevice::synchMirrorMode(){
	...
	for(int i = 0;i < STREAM_TYPE_SIZE;++i){
		bool mirror_support;
		try{
			mirror_support = m_streams[i]->isMirrosSupported();
	...
	for(int i = 0;i < STREAM_TYPE_SIZE;++i){
		if(m_streams[i]->isMirrosSupported() == false){
			break;
		}
...
COpenNI2Generic::CDevice::CStream::CStream(openni::Device& device, openni::SensorType type, openni::PixelFormat format, std::ostream& log)
	:m_device(device), m_type(type), m_format(format), m_log(log), m_strName("Unknown")
{
...
bool COpenNI2Generic::CDevice::CStream::isMirrosSupported()const{
	if(isValid() == false){
...
{ 
	...
	if(rc != openni::STATUS_OK){
		hardware_error = true;
		std::string message = mrpt::format("Failed to grab frame from %s", getName());
